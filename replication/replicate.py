...
def train_model(dataset, hyperparams, best_model_path, ModelClass, epochs=10000, patience=100, halting_step=5):
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % 10 == 0:
            print(f"Epoch: {epoch:03d}, Loss: {train_loss:.4f}, Train Avg Steps: {train_avg_steps:.2f}, Val Acc: {val_acc:.4f}")
...
def main():
    ...
    # If none provided just run the best
    if prop_penalties is None:
        prop_penalties = [hyperparameters["prop_penalty"]]
    ...
    for prop_penalty in prop_penalties:
        hyperparameters["prop_penalty"] = prop_penalty
        for index, seed in tqdm(enumerate(seeds)):
            for i in range(5):
                ...
                print(f"Run for prop-penalty {prop_penalty}, seed index {index}/{len(seeds)-1}, trial {i}/4 finished")
                run.finish()
