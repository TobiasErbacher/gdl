...
class APGCNMappingMoved(torch.nn.Module):
    """
    Adaptive Propagation Graph Convolutional Network.
    Move one of the linear layers after the message passing layer.
    """
    ...
    def __init__(self,
        ...
        dropout: Dropout rate
        """
        super(APGCNMappingMoved, self).__init__()

        # as authors did, we create the mlp before prop.
        layers = [Linear(dataset.data.x.shape[1], hidden[0]), Linear(hidden[0], dataset.num_classes)]

        # we do the propagation with the previous format.
        self.prop = AdaptivePropagation(niter, hidden[0])
    ...
    def forward(self, data, return_propagation_cost=False):
        ...
        x = self.layers[0](self.dropout(x))
        x = self.act_fn(x)
...
class AdaptivePropagationNoWeightedSum(MessagePassing):
    def __init__(self, niter: int, h_size: int, bias=True, **kwargs):
        """
        Adaptive propagation layer.
        ...
        bias: if to add a bias in the halting unit
        """
        super(AdaptivePropagationNoWeightedSum, self).__init__(aggr='add', **kwargs)
...
class APGCNNoWeightedSum(torch.nn.Module):
    """
    Adaptive Propagation Graph Convolutional Network.
    """
    ...
    def __init__(self,
        ...
        dropout: Dropout rate
        """
        super(APGCNNoWeightedSum, self).__init__()
        ...
        # we do the propagation with the previous format.
        self.prop = AdaptivePropagationNoWeightedSum(niter, dataset.num_classes)
